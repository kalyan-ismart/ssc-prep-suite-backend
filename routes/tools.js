...
// FIXED: Complete validation middleware for create/update
const validateTool = [
    body('name')
        .isString()
        .trim()
...
// FIXED: Complete validation middleware for query parameters
...
router.get('/', [optionalAuth, ...validateToolQuery], asyncHandler(async (req, res) => {
    ...
    if (!errors.isEmpty()) {
        return errorResponse(res, 'Invalid query parameters', 422, errors.array());
    }
...
router.post('/add', [auth, adminAuth, ...validateTool], asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (existing) {
            return errorResponse(res, 'Tool name already exists', 409);
        }
        ...
        await logSecurityEvent(req, 'TOOL_CREATED', 'info', {
            toolId: tool._id,
            name: tool.name,
            createdBy: req.user.id
        });
...
router.post('/update/:id', [
    ...
    try {
        ...
        if (req.user.role !== 'admin' && tool.createdBy.toString() !== req.user.id) {
            await logSecurityEvent(req, 'UNAUTHORIZED_TOOL_UPDATE', 'warning', {
                toolId: req.params.id,
                userId: req.user.id
            });
            return errorResponse(res, 'You can only update tools you created', 403);
        }
        ...
        await logSecurityEvent(req, 'TOOL_UPDATED', 'info', {
            toolId: req.params.id,
...
router.delete('/:id', [
    ...
    try {
        ...
        await logSecurityEvent(req, 'TOOL_DELETED', 'info', {
            toolId: req.params.id,
            name: tool.name,
...
router.post('/:id/toggle', [
    ...
    try {
        ...
        await logSecurityEvent(req, 'TOOL_STATUS_TOGGLED', 'info', {
            toolId: tool._id,
